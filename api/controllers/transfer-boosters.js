        ...
        .then(() => {
          ...
          database.query(QUERY_BOOSTERS, [githubUser], (error, result) => {
            if (error) {
              res.status(500);
              res.send(
                "Error: Failed reading sender's boosters from database."
              );
            } else {
              if (result.length === 1) {
                      ...
                      if (error) {
                        ...
                        database.beginTransaction(error => {
                          if (error) {
                            res.status(500);
                            res.send(
                              "Error: Failed writing boosters to database."
                            );
                          } else {
                            database.query(
                              UPDATE_BOOSTERS,
                                ...
                                if (error) {
                                  ...
                                  if (receiverBoostersResult.length === 1) {
                                    ...
                                    database.query(
                                      UPDATE_BOOSTERS,
                                      [
                                        receiverBoosters.strikes +
                                          boosters.strikes,
                                        receiverBoosters.spares +
                                          boosters.spares,
                                        receiverBoosters.doubles +
                                          boosters.doubles,
